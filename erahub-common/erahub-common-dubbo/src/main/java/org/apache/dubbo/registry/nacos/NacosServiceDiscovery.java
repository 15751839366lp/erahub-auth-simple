...
/**
 * 覆盖 dubbo 源码 修复 `dubbo.registry.group` 配置项注册组不生效问题
 *
...
public class NacosServiceDiscovery extends AbstractServiceDiscovery {
    ...
    private final String DUBBO_GROUP = SpringUtils.getProperty("dubbo.registry.group");
    ...
    public void doRegister(ServiceInstance serviceInstance) {
        execute(namingService, service -> {
            ...
            service.registerInstance(instance.getServiceName(), DUBBO_GROUP, instance);
        });
    }
    ...
    public Set<String> getServices() {
        return ThrowableFunction.execute(namingService, service -> {
            ...
            // Or will cause consumer unable to fetch all of the providers from every group
            // Provider's group is invisible for consumer
            ListView<String> view = service.getServicesOfServer(0, Integer.MAX_VALUE, DUBBO_GROUP);
